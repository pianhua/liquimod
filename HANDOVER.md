...
- **技术解决**：
  - 提取并直接内置 XXMI 官方 3.13MB 专属定制版 `d3d11.dll`（SHA256: `6E267C2C...`）、`3dmloader.dll` (20KB) 与 `d3dcompiler_47.dll` (4.92MB)；
  - `deploy_embedded_srmi_suite` 加入二进制文件对比校验，在工作区初始化及每次启动游戏前自动进行静默升级自愈；
  - 优化 `download_and_install_migoto`，始终拉取并安装 `XXMI-Libs-Package` 与 `SRMI-Package`。

### 2.2 100% 原生无感 Win32 Hook 注入拉起游戏（彻底移除第三方 Loader 弹窗）
- **根因分析**：此前工作区未下发 `3dmloader.dll`，导致 Helper 回退至 `spawn_program_with_uac(loader_exe)`，启动游戏时先弹 3DMigoto Loader 窗口及延时。
- **技术解决**：
  - 核心工作区必定下发 `3dmloader.dll`；
  - `commands.rs` 与 `launcher.rs` 全面改写为原生 Win32 Hook 流程（`HookLibrary` $\to$ `CreateProcessW`/`StartProcess` $\to$ `WaitForInjection` $\to$ `UnhookLibrary`）；
  - 支持 `StartProcess` 提权拉起，游戏一气呵成启动并挂载 Mod，用户界面与体验 100% 无感。
