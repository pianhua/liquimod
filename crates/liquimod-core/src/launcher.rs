...
pub struct GameLaunchOptions {
    ...
    pub delay_ms: u64,
    /// 是否在启动前重写 d3dx.ini 的 mode/target。
    /// 提权 helper 路径传 false：普通权限 UI 已在 prepare_managed_runtime
    /// 中按用户选择写好 mode/target/delay,helper 不得覆盖。
    pub sync_d3dx_ini: bool,
}
    ...
    #[cfg(windows)]
    {
        ...
        match cmd.spawn() {
                    ...
                    || e.kind() == std::io::ErrorKind::PermissionDenied =>
            {
                ...
                // 主程序自 v0.7 起以 asInvoker 运行：未提权时必须显式 -Verb RunAs，
                // 否则 PowerShell 子进程只是普通权限，无法再拉起要求提权的目标。
...
/// 1. 自动同步 d3dx.ini 模式与 target 游戏路径（sync_d3dx_ini 开启时）
...
pub fn launch_with_mod(opts: &GameLaunchOptions) -> Result<LaunchResult> {
    launch_with_mod_progress(opts, &mut |_| {})
}

/// 与 [`launch_with_mod`] 相同，额外在生命周期关键点回调阶段码：
/// `hook_ok` / `spawned` / `early_ok` / `early_fail` / `window_seen` /
/// `window_timeout` / `late_ok` / `late_fail` / `unhook`。
/// 提权 helper 据此向普通权限 UI 汇报进度，并在 helper 日志中保留现场。
pub fn launch_with_mod_progress(
    ...
    if opts.sync_d3dx_ini && d3dx_ini.is_file() {
    ...
    #[cfg(windows)]
    {
        ...
        let mut hook = HookSession::new(&opts.injector_dll, &d3d11, &process_name)?;
        stage("hook_ok");
        ...
        stage(if window_seen {
            "window_seen"
        } else {
            "window_timeout"
        });
        let (late_hooked, late_result) = if window_seen {
            let result = hook.wait_for_injection(5)?;
            stage(if result.0 { "late_ok" } else { "late_fail" });
            result
        } else {
            (false, -1)
        };
        tracing::info!(process = %process_name, hooked = late_hooked, "XXMI late Hook verification completed");
        // Hook 必须驻留到早期校验、游戏窗口等待和晚期校验全部完成之后才解除；
        // 这里显式解除(Drop 兜底其余退出路径),保证解除时点不早于晚期校验。
        hook.unhook();
        stage("unhook");
...
impl HookSession {
    ...
    fn unhook(&mut self) {
        ...
        // 解除时点与结果是注入事故排查的关键现场，必须落日志。
        tracing::info!(result = ?result, "XXMI UnhookLibrary completed");
...
mod tests {
    ...
    /// 搭建最小 3DMigoto 工作区与游戏 exe;注入器 DLL 故意缺失,
    /// 让启动流程在 ini 同步之后、Hook 之前失败,从而可以在没有
    /// 真实游戏与 3dmloader.dll 的环境下观察 ini 同步门控行为。
    fn scaffold_workspace(sync_d3dx_ini: bool) -> (tempfile::TempDir, GameLaunchOptions) {
        let temp = tempfile::tempdir().unwrap();
        let migoto_dir = temp.path().join("3DMigoto");
        std::fs::create_dir_all(&migoto_dir).unwrap();
        std::fs::write(
            migoto_dir.join("d3dx.ini"),
            "[Loader]\r\ntarget = placeholder.exe\r\n",
        )
        .unwrap();
        std::fs::write(migoto_dir.join("d3d11.dll"), b"").unwrap();
        let game_exe = temp.path().join("StarRail.exe");
        std::fs::write(&game_exe, b"").unwrap();
        let opts = GameLaunchOptions {
            game_exe,
            migoto_dir,
            injector_dll: temp.path().join("missing-3dmloader.dll"),
            process_name: "StarRail.exe".to_string(),
            work_mode: MigotoWorkMode::Play,
            delay_ms: 0,
            sync_d3dx_ini,
        };
        (temp, opts)
    ...
    #[test]
    fn sync_disabled_leaves_d3dx_ini_untouched() {
        let (_temp, opts) = scaffold_workspace(false);
        let ini_path = opts.migoto_dir.join("d3dx.ini");
        let before = std::fs::read(&ini_path).unwrap();

        let result = launch_with_mod(&opts);

        assert!(result.is_err());
        assert_eq!(std::fs::read(&ini_path).unwrap(), before);
    }

    #[test]
    fn sync_enabled_rewrites_d3dx_ini_target() {
        let (_temp, opts) = scaffold_workspace(true);
        let ini_path = opts.migoto_dir.join("d3dx.ini");
        ...
        assert!(content.contains("StarRail.exe"));
        assert!(!content.contains("placeholder.exe"));
    }

    #[test]
    fn progress_callback_survives_early_failure() {
        let (_temp, opts) = scaffold_workspace(false);
        let mut stages = Vec::new();

        let result = launch_with_mod_progress(&opts, &mut |stage| {
            stages.push(stage.to_string());
        });

        // 注入器缺失在 Hook 之前失败:不得产生 hook_ok 等后续阶段,也不能 panic。
        assert!(result.is_err());
        assert!(!stages.iter().any(|stage| stage == "hook_ok"));
        assert!(!stages.iter().any(|stage| stage == "unhook"));
    }
}
