...
StructuredBuffer<uint> t1 : register(t21);
...
void main(uint3 vThreadID: SV_DispatchThreadID) {
    ...
    uint index = t1[vThreadID.x];

    float4 pose_0 = (t2[index + offset][0].xyzw);
    float4 pose_1 = (t2[index + offset][1].xyzw);
    float4 pose_2 = (t2[index + offset][2].xyzw);
